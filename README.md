...
5분마다 approvals.yml → ① 승인 수거 + "승인 상태 변경 ✅" 확인 답장
                      ② 슬롯 시각이 지났는데 안 올라간 승인 슬롯을 따라잡아 게시
                         (하나씩, 직전 게시로부터 10분 이상 띄워서)
                      ③ 오늘 카드가 없으면 render 를 깨움(ET 7:20-17:00, 30분 간격 최대 4회)
                      ④ relay 잡이 5분 뒤 자기 자신을 다시 발동해 체인을 잇는다
...
```json
{
  ...
  "failed": { "art": { "error": "...", "at": "..." } },
  "preview_sent": true, "render_kicks": 2, "render_kick_at": "..."
}
```

- `approvals`: `true`=승인, `false`=건너뛰기, `null`=대기. **`is True`일 때만 게시** (변경 금지).

## 3-1. cron 을 믿지 않는 설계 (catchup + relay)

GitHub Actions 의 cron 은 밀리는 정도가 아니라 **대부분 발화하지 않는다.**
2026-08-27~29 실측으로 `*/10` 스케줄이 하루 96회 예정 중 2~3회만 돌았고,
2026-08-29(토)에는 아침에 render cron 이 한 번도 발화하지 않아 카드가 통째로
누락됐다. 그래서 시각에도, cron 에도 의존하지 않는다:

- **미리보기**는 카드가 커밋되는 push 로 발동한다 (cron 은 백업)
- **approvals 체인**: approvals.yml 의 `relay` 잡이 5분을 기다렸다가 자기 자신을
...
- **게시**는 그 체인이 `publisher.py --catchup` 으로 따라잡는다. 승인됐고 슬롯
  ...
  인스타에는 10분 미만 간격으로 연속 게시되지 않는다 - Meta 스팸 감지 회피)
- **렌더링**은 오늘 카드가 없으면 체인이 ET 7:20-17:00 사이에 render 를
  30분 간격으로 최대 4번 깨운다. render 는 카드가 이미 있거나 Drive 에 spec 이
  없으면 스스로 조용히 건너뛰므로 헛방이 안전하다
- 게시 실패는 `state/<날짜>.json` 의 `failed` 에 기록되고 **그날은 자동 재시도하지 않는다**
  (Meta 차단 중 반복 재시도로 차단을 키우는 것을 막는다). 재시도는 수동 dispatch 로

따라서 슬롯 시각보다 최대 5-10분 늦게 나갈 수 있고, cron 이 거의 죽어도 게시는 계속된다.
...
- 승인은 5분 주기 approvals 체인이 수거하며, 반영되면 **"승인 상태 변경: ✅ FREE" 확인
...
| 미리보기가 안 옴 | render cron 미발화 (2026-08-29 실사례) 또는 렌더 실패 | `gh workflow run approvals.yml` 로 체인을 깨우면 render 를 대신 발동한다 |
| 승인 확인 답장이 안 옴 | approvals 체인이 끊김(cron 미발화) 또는 다른 채팅에서 누름 | 10분 기다려도 없으면 `gh workflow run approvals.yml` (체인이 다시 살아난다) |
| `not approved ... nothing published` | 승인 안 함(정상) | 승인하면 5-10분 안에 catchup 이 올린다 |
| 슬롯 시각이 지났는데 안 올라감 | cron 미발화 | 5-10분 내 catchup 이 처리. 그래도 안 되면 `state` 의 `failed` 확인 |
